{{-- <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg"> --}}
     <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-indigo-100">
          ...
          <div class="p-6">
               ...
               <form id="productTypeForm" action="{{ route('product-type.store') }}" method="POST" class="mb-8">
                    @csrf
                    <input type="hidden" id="productTypeId" name="id" value=""> <!-- Hidden input for ID -->
                    ...
                    <div class="grid md:grid-cols-12 gap-6 items-end">
                         ...
                         <div class="md:col-span-5">
                              <label for="description" class="block text-xs font-medium text-indigo-700 mb-1">Description</label>
                              <div class="relative">
                                   <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-indigo-500 absolute left-3 top-1/2 transform -translate-y-1/2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" />
                                   </svg>
                                   <input type="text" class="pl-10 w-full bg-indigo-50 border-0 border-b-2 border-indigo-300 focus:border-indigo-500 focus:ring-0 focus:bg-white transition-all duration-200 text-sm" id="description" name="description" placeholder="Brief description">
                              </div>
                         </div>
                         ...
                         <div class="md:col-span-3">
                              <button type="submit" class="w-full h-full flex items-center justify-center bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-medium py-2 px-4 rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 text-sm">
                                   <span id="formSubmitButtonText">Add Product Type</span>
                              </button>
                         </div>
                    </div>
               </form>
                         ...
                         <table class="w-full text-sm">
                              <thead class="bg-gradient-to-r from-indigo-100 to-purple-100 sticky top-0 z-10"> <!-- Made header sticky -->
                                   <tr>
                                        ...
                                        <th class="py-2 px-4 text-left text-xs font-semibold text-indigo-800">Description</th>
                              ...
                              <tbody class="divide-y divide-indigo-100">
                                   ...
                                   @forelse ($productTypes as $type)
                                        <tr class="hover:bg-indigo-50 transition-colors duration-200">
                                             ...
                                             <td class="py-2 px-4 text-xs text-gray-600">{{ $type->description }}</td>
                                             <td class="py-2 px-4 text-right">
                                                  <div class="flex justify-end space-x-2">
                                                       ...
                                                       <form action="{{ route('product-type.destroy', $type->id) }}" method="POST" class="inline">
                                                            @csrf
                                                            @method('DELETE')
...
<script>
     ...
     const formMethod = document.getElementById('formMethod'); // Hidden input for method
     ...
     editButtons.forEach(button => {
          button.addEventListener('click', () => {
               ...
               form.action = `/settings/product-action/update/${id}`;
               formMethod.value = 'PUT'; // Set the method to PUT
               submitButtonText.textContent = 'Update Product Type';
          });
     });
     });
</script>
